...
; DESCHANEL_v4.asm
;
; Ce programme récupère un mot de passe depuis un fichier puis prend en 
...
section .bss                  ; Définition des variables en lecture et écriture
  ...
  password: resb 22           ; Mot de passe récupéré depuis le fichier
;--------------------------------------------------
...
;
; Input:
;   - 1: Chaine de caractères à comparer avec le mot de passe
; Output:
...
compare_password:
  ...
  mov edx, ARG_1                  ; Récupération de la chaine passée en paramètre
  ...
  _strcmp_loop:
    ...
    je _strcmp_end
    cmp BYTE [edx + ecx], 0     ; Si la fin du mot de passe est atteint, la fonction quitte
    je _strcmp_end

    mov al, BYTE [password + ecx]
    cmp al, BYTE [edx + ecx]    ; Comparaison entre les deux caractères
...
_start:
  ...
  call parse_shadow_file                    ; Le mot de passe est récupéré depuis un fichier
